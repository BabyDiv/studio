...
export default function Questions() {
  ...
  return (
    <section id="questions" className="questions flex flex-col px-[8%]">
      ...
      <p className="text-[#FFFFFF] questions__description m-0 text-center mb-8">
              ...
              {section.items.map((item, questionIndex) => {
                ...
                return (
                  <li key={questionIndex}>
                    <button
                      className="w-full text-left flex justify-between items-center py-[18px] px-0 cursor-pointer bg-transparent border-none"
                      onClick={() => toggleQuestion(sectionIndex, questionIndex)}
                    >
                      <p className="text-[#FFFFFF] text-[16px] m-0">{item.question}</p>
                      <div className={`w-[24px] h-[24px] flex-none transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-[24px] h-[24px] fill-[#716F6F]">
                    ...
                    <div
                      ref={el => { 
                        answerRefs.current[sectionIndex * 10 + questionIndex] = el; 
                      }}
                      className="overflow-hidden transition-all duration-700 ease-in-out"
                      style={{
                        maxHeight: isOpen
                          ? answerRefs.current[sectionIndex * 10 + questionIndex]?.scrollHeight + 'px'
                          : '0px',
                      }}
                    >
                      <p className="px-6 pb-4 mt-[0px] text-[#FFFFFF] questions__answer text-[16px]">{item.answer}</p>
                    </div>
                  </li>
                );
              })}
